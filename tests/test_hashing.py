...
def test_sha1():
    assert Chepy("A").sha1().out == "6dcd4ce23d88e2ee9568ba546c007c63d9131c1b"
...
def test_md2():
    assert Chepy("A").md2().out == "08e2a3810d8426443ecacaf47aeedd17"


def test_md4():
    assert Chepy("A").md4().out == "d5ef20eeb3f75679f86cf57f93ed0ffe"


def test_md5():
    assert Chepy("A").md5().out == "7fc56270e7a70fa81a5935b72eacbe29"
...
def test_shake_128():
    assert (
        Chepy("A").shake_128(128).out.decode()
...
def test_ripemd_160():
    assert Chepy("A").ripemd_160().out == "ddadef707ba62c166051b9e3cd0294c27515f2bc"
...
def test_crc8_checksum():
    assert Chepy("abc").crc8_checksum().out == "5f"


def test_crc16_checksum():
    assert Chepy("a").crc16_checksum().out == "e8c1"


def test_crc32_checksum():
    assert Chepy("a").crc32_checksum().out == "e8b7be43"


def test_hmac_hash():
    assert (
        Chepy("abc").hmac_hash("", "md5").out == "dd2701993d29fdd0b032c233cec63403"
    )
    assert (
